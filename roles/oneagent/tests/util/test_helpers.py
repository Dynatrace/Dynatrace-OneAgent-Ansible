...
def enable_for_system_family(family: str) -> Callable:
    def func_wrapper(func):
        ...
        def params_wrapper(*args, **kwargs):
            config: AnsibleConfig = _get_param_by_name(
                "configurator", **kwargs)
            platforms: PlatformCollection = _get_param_by_name(
                "platforms", **kwargs)
...
def perform_operation_on_platforms(
...
def set_ca_cert_download_params(
        config: AnsibleConfig,
        installer_server_url: str) -> None:
...
def set_installer_download_params(
        config: AnsibleConfig,
        installer_server_url: str) -> None:
    config.set_common_parameter(
        config.ENVIRONMENT_URL_KEY,
...
def run_deployment(
        ...
        ignore_errors: bool = False) -> DeploymentResult:
...
def check_agent_state(
    ...
    result = wrapper.file_exists(
        platform, address, get_oneagentctl_path(platform))
...
def check_download_directory(
    ...
    logging.debug("Platform: %s, IP: %s", platform, address)
    download_path: Path = get_platform_argument(
        platform, unix_path, windows_path)
    installer_path = download_path / f"{INSTALLER_PARTIAL_NAME}*"
    assert wrapper.directory_exists(
        platform, address, download_path).returncode == 0
    assert wrapper.file_exists(
        platform,
        address,
        installer_path).returncode == 0 if exists else 1
